...
vector<vector<char>> alphDivider(int numStates, vector<char> alphArray){
  ...
  transitionList.resize(sublistEntriesNum,alphArray.size());
  
  vector< vector<int> >::iterator row;
  vector<int>::iterator col;
  for (row = transitionList.begin(); row != transitionList.end(); row++) {
    for (col = row->begin(); col != row->end(); col++) {
        // do stuff ...
        int randIndex = rand() % 256;
        transitionList[i][j] = alphArray[randIndex];
...
void markovCreatorFINAL_FINAL(int numMcs, int mcStateNum){
    vector<Automata> mcs;
    mcs.resize(numMcs);
    //Call the alphabet splitter


    //TODO
    //Add proper string transition splits for each node's ingoing, outgoing, and stay transitions
	...
	for (int j = 0; j < numMcs; j++){
	  ...
	  if (i != j){
	      ...
	      tOutDynamicVars[i] = new STE(out_path,str+to_string(i),""); // Check what string will cause this transition
	      mcs[h].addEdge(r+to_string(i),out_path);
	      mcs[h].addEdge(out_path,r+to_string(j));
	      ...
	      tInDynamicVars[i] = new STE(out_path,str+to_string(i),"");
	      mcs[h].addEdge(r+to_string(j),in+path);
	      mcs[h].addEdge(in+path,r+to_string(i));
